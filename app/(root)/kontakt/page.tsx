...
export default function ContactPage() {
    ...
    const inputClassNames =  "w-full bg-[color:var(--color-neutral)]  rounded-xl p-3 focus:ring-1 focus:ring-[color:var(--color-accent)] disabled:bg--[color:var(--color-neutral)] disabled:cursor-not-allowed"
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setToast({
                message: typeof error === "string" ? error : "Ein unbekannter Fehler ist aufgetreten.",
                type: "error"
            });
        } finally {
            setIsLoading(false);
        ...
        <div className="innerBox  ">
            <div className=" grid grid-cols-1 md:grid-cols-2 md:gap-4 lg:gap-10">
                <div className="flex flex-col ">
                    ...
                    <a href="mailto:testemail@projekt.de" className="flex items-center gap-2  py-4">
                        <span>E-Mail:</span>
                        <span className={"text-[color:var(--color-secondary)] hover:text-[color:var(--color-primary)]"}>testemail@projekt.de</span>
                    </a>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 ">
                        ...
                        <div>
                            ...
                            />
                            {formData.email && (
                                <label htmlFor="email" className="p-1 block text-sm font-medium text-gray-400">E-Mail</label>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            ></textarea>
                            {formData.message && (
                                <label htmlFor="message" className="pl-1 block text-sm font-medium text-gray-400">Nachricht</label>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 place-items-center">
                            ...
                            {isLoading && (
                                <div className="flex items-center mt-2">
                                    <div
                                        className="w-5 h-5 border-2 border-[color:var(--color-text)] border-t-transparent rounded-full animate-spin"></div>
                                    <span className="ml-2 text-[color:var(--color-text)]">Wird gesendet...</span>
                                </div>
                            )}
                        </div>
