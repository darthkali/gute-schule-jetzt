...
const MastodonShareButton = ({ text, shareUrl, className }: { text: string; shareUrl: string, className: string }) => {
    ...
    const handleClick = () => {
        ...
        if (instance) {
            const instanceUrl = instance.trim().replace(/^https?:\/\//, '').replace(/\/$/, '');
            if (!instanceUrl.match(/^[a-z0-9.-]+\.[a-z]{2,}$/i)) {
                alert("Bitte gib eine gültige Mastodon-Instanz ein (z. B. mastodon.social)");
                return;
            }
